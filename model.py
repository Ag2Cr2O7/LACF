...
class LACF(nn.Module):
    ...
    def auto_augment_ED(self, gnn,i, is_norm=False):
        ...
        if is_norm:
            A_tensor = torch_sparse.SparseTensor(row=self.all_h_list, col=self.all_t_list, value=zqg,sparse_sizes=self.A_in_shape).cuda()
    ...
    def auto_augment_emb(self,i,f_emb):
    ...
    def inference1(self):
        ...
        for i in range(0, self.n_layers):
            ...
            zqg1 = self.auto_augment_ED(all_embeddings1[i],i, is_norm=True)
            ...
            new_emb=self.auto_augment_emb(i,all_embeddings2[i])
            gnnf = torch_sparse.spmm(self.G_indices, self.G_values, self.A_in_shape[0], self.A_in_shape[1],new_emb)
    ...
    def cos_loss(self,a,b):
        return nn.functional.cosine_similarity(a,b, dim=1).sum()
    ...
    def forward(self, users, pos_items, neg_items):
        ...
        cse_loss = self.ssl_reg * self.cse_loss(users, pos_items,self.zuu,self.zut,self.zii,self.zit,self.temp)
        svd_loss= self.lrec * self.res_loss(users, pos_items,self.eu,self.eu1,self.ei,self.ei1,self.rt)+\
    ...
    def res_loss(self, user,pos,u,u2,i,i2,t):
        ...
        recon_lossu = self.ssl_con_loss(recon_u, recon_u2, t)
        recon_lossi = self.ssl_con_loss(recon_i, recon_i2, t)
        recon_loss=recon_lossu+recon_lossi
        return recon_loss

    def predict(self, users):
        u_embeddings = self.eu[torch.LongTensor(users).cuda()]
